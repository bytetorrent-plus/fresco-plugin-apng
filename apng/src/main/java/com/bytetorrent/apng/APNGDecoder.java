    ...
    protected Rect read(APNGReader reader) throws IOException {
        ...
        for (Chunk chunk : chunks) {
            if (chunk instanceof ACTLChunk) {
                ...
                APNGFrame frame = new APNGFrame(reader, (FCTLChunk) chunk, frames.size(), this);
...
//                    mLoopCount = 1;
    ...
    @Override
    protected synchronized void renderFrame(Frame frame, Bitmap inBitmap) {
        ...
        try {
            ...
            //开始真正绘制当前帧的内容
            if(inBitmap == null){
                inBitmap = obtainBitmap(frame.frameWidth, frame.frameHeight);
            }

            frame.draw(canvas, paint, sampleSize, inBitmap, getWriter());
